...
public class UserSignInSignUpController {
	...
	public ResponseEntity<?> signinUser(@RequestBody @Valid SigninRequest reqDTO) {
		...
		// => auth success
		return ResponseEntity.ok(new SigninResponse(utils.generateJwtToken(verifiedAuth),
				utils.getAuthoritiesInString(userDetails.getAuthorities()), "Successful Authentication!!!"));
//		return ResponseEntity.ok(new SigninResponse(utils.generateJwtToken(verifiedAuth),
//					utils.getAuthoritiesInString(userDetails.getAuthorities()),"Successful Authentication!!!"));
